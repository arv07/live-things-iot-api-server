...
class SocketIOController extends Controller
{
    ...
    //Get device associated with user
    public function getDeviceAssociated($user_token, $id_device)
    {
        try {
            $user = User::where('user_token', $user_token)->first();
            ...
            return response()->json(['state' => 'ok', 'message' => 'ok', 'data' =>  $user[0]->devices[0], 'levelNotication' => 1], 200);
